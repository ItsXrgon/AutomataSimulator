...
class AUTOMATASIMULATOR_API DeterministicFiniteAutomaton : public FiniteAutomaton {
  private:
	 ...
	 */
	bool checkAlphabetValidity(const std::set<std::string> &inputAlphabet) const;
  ...
  public:
	 ...
	 */
	void setInputAlphabet(const std::set<std::string> &inputAlphabet) override;
	 ...
	 * @throws InvalidAlphabetException If the alphabet contains an epsilon (empty string).
	 */
	void addInputAlphabet(const std::set<std::string> &inputAlphabet) override;
